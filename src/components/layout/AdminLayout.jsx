...
export default function AdminLayout() {
  ...
  const base = `/branch/${branchId}/admin`;

  const navItems = [
    { to: base, icon: LayoutDashboard, label: '대시보드', end: true },
    { to: `${base}/members`, icon: Users, label: '회원 관리' },
    { to: `${base}/attendance`, icon: CalendarCheck, label: '출석 관리' },
    { to: `${base}/fees`, icon: Wallet, label: '회비 관리' },
    { to: `${base}/programs`, icon: BookOpen, label: '프로그램 관리' },
    { to: `${base}/toys`, icon: Package, label: '장난감 관리' },
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      <aside className="w-64 bg-white border-r border-gray-200 fixed h-full flex flex-col">
        <div className="p-6">
          <h1 className="text-xl font-bold text-indigo-600">수눌음 관리자 - {currentBranch?.name}</h1>
        ...
        <div className="p-4 border-t border-gray-200 space-y-1">
          <NavLink to={`/branch/${branchId}`} className="flex items-center gap-3 px-6 py-3 text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors">
