...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('enrollments').select('id', { count: 'exact', head: true }).eq('branch_id', branchId).eq('member_id', profile.id).eq('status', 'enrolled'),
  ...
  const quickLinks = [
    { to: `/branch/${branchId}/programs`, icon: BookOpen, label: '프로그램 둘러보기', desc: `${stats.programs}개 프로그램 모집 중`, color: 'bg-indigo-500' },
    { to: `/branch/${branchId}/children`, icon: Baby, label: '자녀 관리', desc: `${stats.children}명 등록`, color: 'bg-pink-500' },
    { to: `/branch/${branchId}/my/attendance`, icon: CalendarCheck, label: '출석 내역', desc: '올해 출석 기록 보기', color: 'bg-green-500' },
    { to: `/branch/${branchId}/my/fees`, icon: Wallet, label: '회비 내역', desc: '납부 현황 확인', color: 'bg-amber-500' },
    { to: `/branch/${branchId}/toys`, icon: Package, label: '장난감 대여', desc: '장난감 목록 및 대여 신청', color: 'bg-orange-500' },
    { to: `/branch/${branchId}/rental-request`, icon: Building2, label: '대관 신청', desc: '시설 대관 신청하기', color: 'bg-teal-500' },
  ];
      ...
      {stats.enrollments > 0 && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-6">
          <p className="text-indigo-800 text-sm font-medium">
            ...
            <Link to={`/branch/${branchId}/my/enrollments`} className="ml-2 underline">확인하기</Link>
          </p>
        </div>
      )}
